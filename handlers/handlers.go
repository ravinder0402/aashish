...
func StartingPage(c *gin.Context) {
	var bookRequests []models.BookInventory
	var bookTitles []string
	if err := db.DB.Find(&bookRequests).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch issue requests"})
		return
	}
	for _,book := range bookRequests{
		bookTitles=append(bookTitles,book.Title)
	}
	
	c.JSON(http.StatusOK, bookTitles)
...
func RemoveBook(c *gin.Context) {
	...
	if book.AvailableCopies < book.TotalCopies {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Cannot remove copy book is issued to someone"})
		return
	}
